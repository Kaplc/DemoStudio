...
export class BlueprintPreviewManager {
   ...
   *  - property 类 op → 把 ops 逐个应用到 _jsonTree 深拷贝（复用服务层 runOp 同一套 ops 实现），
   *    再把结果交给 _applySnapshotInPlace 回写到活动 Actor（按节点名匹配 + editableProperty set）。
   */
  applyEditOps(
    assetPath: string,
    ...
    if (this._currentBlueprintKey !== key || !this._jsonTree || !this._actorJsonMap) return false
    ...
    for (const { op, params } of ops) {
      ...
      if (!res.ok || !res.asset) {
        logger.warn(`[BlueprintPreview] 快速通道 op 被拒（回退重建）: ${op} → ${key}: ${res.error}`)
        return false
      }
      tree = res.asset as unknown as Record<string, unknown>
    }
    if (JSON.stringify(tree) === before) {
      logger.info(`[BlueprintPreview] 快速通道内容无变化，跳过: ${key}`)
      return true
    }
    ...
    if (!applied) {
      logger.info(`[BlueprintPreview] 快速通道原地应用失败（回退重建）: ${key}`)
      return false
    }
    ...
    void this.syncWorkingCopy()
    logger.info(`[BlueprintPreview] 快速通道就地应用 ${ops.length} ops: ${key}`)
    return true
  }
