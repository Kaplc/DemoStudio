/**
 * ScriptComponent — 资产「挂载脚本」组件（通用，Unity MonoBehaviour 挂载点）
 *
 * 数据驱动：在蓝图资产（3D 建筑 / UI widget 等）的任意节点加上本组件，
 * 并在 properties.script 指定已注册的脚本 id，引擎实例化该节点时即自动创建
 * 脚本实例、注入宿主 Actor 并接入生命周期（onStart / onUpdate / onDestroy）。
 * 从而把「结构」（资产）与「行为」（脚本）解耦。
 *
 * 数据配置：
 *   { baseClass: 'ScriptComponent', properties: { script: 'gameplay/base/ClashBuilding', args?: {...} } }
 *
...
export class ScriptComponent extends Component<Actor> {
  /** 已注册的脚本 id（路径式，如 'gameplay/base/ClashBuilding'） */
  public script = ''
  /** 传给脚本 onStart 的可选参数 */
  ...
  constructor(owner: Actor, name = 'ScriptComponent') {
    super(owner)
    this.name = name
  }
  ...
  override BeginPlay() {
    super.BeginPlay()
    if (!this.script) {
      logger.warn(`[ScriptComponent] "${this.owner.name}" 未配置 script，跳过`)
      return
    }
    const inst = ScriptRegistry.create(this.script)
    if (!inst) {
      logger.error(
        `[ScriptComponent] 脚本 "${this.script}" 未注册（owner="${this.owner.name}"）。已注册: [${ScriptRegistry.getRegisteredIds().join(', ')}]`,
      )
      return
    }
    ...
    try {
      ...
      logger.info(`[ScriptComponent] 脚本 "${this.script}" 已挂载到 "${this.owner.name}"`)
    } catch (e) {
      logger.error(`[ScriptComponent] 脚本 "${this.script}" onStart 抛错: ${(e as Error).message}`)
    }
  }
  ...
  override EndPlay() {
    ...
    try {
      ...
      logger.error(`[ScriptComponent] 脚本 "${this.script}" onDestroy 抛错: ${(e as Error).message}`)
    }
