/**
 * ActorComponent — Actor 组件基类（原 Component 更名）
 *
 * 可附加到 Actor 上的行为模块，模仿 UE ActorComponent。
 * 继承 BObjectComponent（生命周期）+ AObjectComponent（基础能力），
 * 泛型 T 限定为 Actor 子类：渲染类组件可访问 owner.root。
 *
 * 本类承载原 Component 的全部能力：
 *  - 可编辑属性体系（EditableProperty / getEditableProperties / getPersistentProps）
 *  - Inspector 属性展示（getProperties）
 *  - 序列化（serialize）
 *
 * 分层：
 *   AObjectComponent（基础能力：owner/启停/序列化/展示）
 *    └── BObjectComponent（+ BeginPlay/Tick/EndPlay/OnDrawGizmos 生命周期）
 *         └── ActorComponent（本类：泛型限定 Actor + 可编辑属性体系）
 */
