...
5. **结合关联文档**：涉及战斗/关卡玩法时对照 `doc/battle_system.md` / `doc/level_system.md` 确认既有设计意图。
...
- DO NOT 直接修改代码——审查只报告违规与建议；**除非用户明确说"顺便修了/直接改"**，才允许编辑
...
完成时输出：审查目标与范围 → 符合项清单 → 违规项（按严重度，含规范条款+代码位置+修复建议）→ 风险/存疑项 → 修复建议汇总表。
