...
4. **输入**：BeginPlay 遍历子树把 `ClickableComponent.layer` 切 `'world'`，UIButton 自动点击层（透明 mesh）即被主相机射线命中。~~⚠️ 语义差异要文档化：screen 层的 `block`"消费点击"在 world 层不存在~~ **【2026-09-05 已实现，V2】**：PhySys 世界层改为几何仲裁（UI 候选与 3D 分池：alwaysOnTop 面板树对 3D 有视觉优先权"所见即所点"，其余取射线最近，注册顺序不再参与归属）；`hit-test: block` 落到节点 marker 块，引擎为 marker 懒创建 UI_HIT_LAYER 透明射线 mesh（纯容器/透明遮罩也能拦），经 `__dsWorldUI` 分流 + 主相机射线检测参与世界层拦截。面板要挡身后点击：给节点写 `hit-test: block`（先例 building_info `.Card`；详见 doc/engine/physics_system.md 踩坑 8）。
