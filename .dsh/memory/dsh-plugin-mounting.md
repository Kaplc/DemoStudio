---
name: dsh-plugin-mounting
description: DSH 插件挂载机制与 dsh-memory/dsh-sync 插件、profile 启动链路的完整结论
type: project
---
# DSH 插件挂载与 profile 机制（2026-08-30 验证结论）

## 插件挂载三要素（home 在 ~/.dsh）
1. **编译**：插件目录 `npm run build` 产出 `dist/`（如 `harness/dsh-memory`、`harness/dsh-sync`、`harness/dsh-engine-tools`）
...
## 已挂插件
- `@demostudio/dsh-memory`：记忆系统（memoryDir 钉到 `E:/DemoStudio/.dsh/memory`）
- `@demostudio/dsh-sync`：启动时把 home 的 `.agent-presets`/`skills`/`profiles`/`memory` 同步到项目 `.dsh`（sha1 内容比对，变化才写，跳过 node_modules）
...
## dsh-engine-tools 修复记录（2026-08-30）
- 目录原名 `harness/dsh-plugin`，2026-08-30 改为 `harness/dsh-engine-tools`（与包名 @demostudio/dsh-engine-tools 一致），junction 目标已同步更新
