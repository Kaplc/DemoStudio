...
**Why:** 纯自觉保存实测触发/写入都太弱（冷通道盲区 + 回合末提醒被 ds-memory 独占 + 更新旧经验比新建更易漏）；这是对 2026-09-08 裁撤"LLM 自动提炼"决策的**补充而非回退**——判定永远在主 agent，插件只给触发信号，零模型请求，`harness_no_llm_design` 依然成立。
