/**
 * deploy_plugin — 一键部署 DSH 插件
 *
 * 完整流程：build → junction → patch → validate
 */
...
export const deployPluginTool = defineTool({
  ...
  execute: async (args: { directory: string; config?: Record<string, unknown>; forceBuild?: boolean }) => {
    ...
    if (!fs.existsSync(pkgPath)) {
      return { success: false, message: `插件目录不存在或缺少 package.json: ${pluginDir}` }
    }

    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'))
    const pkgName = pkg.name
    if (!pkgName) return { success: false, message: 'package.json 缺少 name 字段' }
