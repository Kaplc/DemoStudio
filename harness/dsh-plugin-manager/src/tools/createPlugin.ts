...
export const createPluginTool = defineTool({
  ...
  parameters: {
    name: { type: 'string', required: true, description: '插件短名（如 dsh-screenshot），会自动加 @demostudio/ 前缀' },
    description: { type: 'string', required: true, description: '插件功能描述' },
    directory: { type: 'string', description: '自定义目录名（可选，默认用 name）' },
  },
  output: {
    schema: {
      type: 'object',
  ...
  execute: async (args: { name: string; description: string; directory?: string }) => {
    ...
    const pluginDir = path.resolve('harness', dirName)
