...
export function composeAssociateMessage(
  ...
  for (const hit of hits) {
    ...
    const prefixTag = hit.prefix === undefined ? '' : ` · prefix [${hit.prefix.join(', ')}]`
  ...
  const lines = [
    ...
    '',
    '以下记忆的 prefix 声明了当前读取的文件，全文已自动加载（同一会话内不重复注入）：',
    '',
    blocks.join('\n\n'),
    '',
...
export function registerAssociator(ctx: Context, options: AssociatorOptions): () => void {
  ...
  /** 主 agent 的待注入记忆（fileName → 触发路径与命中文件列表，仅首触发展示用）。 */
  ...
  const projectHits = async (agent: Agent, rawPaths: readonly string[]): Promise<void> => {
    try {
      ...
      const candidates = headers.filter(header => header.prefix !== undefined && header.prefix.length > 0)
      ...
      for (const rawPath of rawPaths) {
        ...
        for (const header of candidates) {
          const files = header.prefix
          if (files === undefined || files.length === 0) continue
          if (injected?.has(header.filename)) continue // 本会话已注入过，不再触发
          ...
          if (!pending.has(header.filename)) {
            pending.set(header.filename, { triggerPath: relPath, prefix: files })
          }
        }
      }
    } catch (error) {
