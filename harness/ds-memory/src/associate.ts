/**
 * 记忆自动联想（prefix 路径召回）——"进行到哪块工作内容就自动想起对应记忆"。
 *
 * 机制与 ds-instructions 同构但刻意轻量：
 ...
 *   求值记忆 frontmatter 声明的 `prefix:` 表达式：支持代码风格 `||`（任一路径
 ...
 *
 * 说明（对齐 memoryTypes.ASSOCIATE_LOAD_TEXT 的措辞）：
 * 注入的是整篇正文——因此写记忆时声明 prefix 的正文必须精炼。
 *
 * @module associate
 */
...
import { MAX_MEMORY_CONTENT_CHARS, parsePrefixExpr } from './memoryTypes.js'
...
/** 单次联想注入的总字符预算（防记忆库膨胀后注入爆炸）。 */
export const MAX_ASSOCIATE_TOTAL_CHARS = 32_000

/** 记忆 frontmatter 头部扫描结果的 TTL（毫秒）：联想频次低，无需更细的失效。 */
const SCAN_TTL_MS = 5_000

/** 成功执行后会使记忆库变化的工具（结果确认成功后使扫描缓存失效）。 */
const MEMORY_MUTATOR_TOOLS: ReadonlySet<string> = new Set([
  'memory_write',
  'memory_forget',
  'memory_review',
])
 ...
 */
export function matchMemoryPrefix(relPath: string, prefix: string | undefined): boolean {
...
export function evalPrefixGroups(groups: readonly (readonly string[])[], relPath: string): PrefixGroupEval {
  ...
  for (const group of groups) {
    const rest = group.filter(term => !matchMemoryPrefix(relPath, term))
...
export interface AssociateComposeResult {
  /** 注入用的完整消息文本（无可注入条目时为 ''）。 */
  text: string
  /** 实际装入消息的记忆文件名（预算内，按传入顺序截断）。 */
  included: string[]
  /** 因预算截断未装入的数量。 */
  omitted: number
}

/** 单条记忆正文的注入截断（复用检索侧的 8000 字符上限）。 */
...
/**
 * 组装"自动联想记忆"注入文本（纯函数，便于单测）。
 * @param hits - 命中记忆，调用方需按 mtime 新→旧排好序。
 * @param triggerPath - 触发联想的被读路径（项目根相对，仅展示）。
 * @param now - 当前时间戳。
 * @param maxTotalChars - 总字符预算。
 */
export function composeAssociateMessage(
  ...
  for (const hit of hits) {
    ...
    const ageText = memoryAge(hit.mtimeMs, now)
    const freshness = memoryFreshnessText(hit.mtimeMs, now)
    const typeTag = hit.type === undefined ? '' : ` [${hit.type}]`
    const prefixTag = hit.prefix === undefined ? '' : ` · prefix ${hit.prefix}`
    const warnTag = freshness === '' ? '' : ' · ⚠️ 可能过期'
    const header = `### ${hit.fileName}${typeTag}（${ageText}${prefixTag}${warnTag}）`
  ...
  const lines = [
    `## 自动联想记忆（读取 \`${triggerPath}\` 触发）`,
    '',
    '以下记忆在 frontmatter 声明了匹配当前读取路径的 prefix，全文已自动加载（同一会话内不重复注入）：',
    '',
    blocks.join('\n\n'),
    '',
    '> 记忆是时点观察：引用其中的 file:line / 代码行为前先与当前代码核对；与当前事实冲突时以当前代码为准，并用 memory_write 更新该记忆。',
  ]
  if (omitted > 0) {
    lines.push('', `[预算：其余 ${omitted} 条匹配记忆超出单次注入字符上限未加载；需要时可用 memory_search 按文件名检索。]`)
  }
...
/**
 * 创建并注册记忆联想器。事件监听随 ctx 生命周期卸载；无定时器/外部句柄。
 * @returns 注销函数（幂等，供显式卸载或测试清理）。
 */
export function registerAssociator(ctx: Context, options: AssociatorOptions): () => void {
  const logger = ctx.logger('ds-memory:associate')
  const trackedTools = new Set(
    (options.trackedTools ?? DEFAULT_ASSOCIATE_TOOLS)
      .filter((name): name is string => typeof name === 'string' && name.trim() !== ''),
  )

  // ── 记忆头部（prefix/mtime）扫描缓存：TTL + 记忆变更工具成功后强制失效 ──
  let headerCache: { at: number; headers: MemoryHeader[] } | undefined
  const invalidateHeaders = (): void => {
    headerCache = undefined
  }
  const memoryHeaders = async (): Promise<MemoryHeader[]> => {
    const now = Date.now()
    if (headerCache !== undefined && now - headerCache.at < SCAN_TTL_MS) {
      return headerCache.headers
    }
    const headers = await scanMemoryFiles(options.memoryDirectory)
    headerCache = { at: now, headers }
    return headers
  }

  /** 主 agent 的已注入记忆（fileName → 触发路径，仅首触发展示用）。 */
  const pendingHits = new WeakMap<Agent, Map<string, { triggerPath: string; prefix: string }>>()
  /** 主 agent 的已注入记忆文件名集合（去重：同会话同记忆只注入一次）。 */
  ...
  ctx.on('tools/result', (exec: ToolExecution, result: ToolExecutionResult) => {
    try {
      ...
      // 记忆库变更工具成功 → 头部缓存失效
      if (MEMORY_MUTATOR_TOOLS.has(exec.name)) invalidateHeaders()
  ...
  const projectHits = async (agent: Agent, rawPaths: readonly string[]): Promise<void> => {
    try {
      ...
      const candidates = headers.filter(header => parsePrefixExpr(header.prefix ?? '') !== undefined)
      ...
      for (const rawPath of rawPaths) {
        ...
        for (const header of candidates) {
          if (injected?.has(header.filename)) continue // 本会话已注入过，不再累计/触发
          const groups = parsePrefixExpr(header.prefix!)!
          const progressMap = andProgress.get(agent)
          const remaining = progressMap?.get(header.filename) ?? groups.map(group => [...group])
          const evaluation = evalPrefixGroups(remaining, relPath)
          if (evaluation.triggered) {
            progressMap?.delete(header.filename)
            let pending = pendingHits.get(agent)
            if (pending === undefined) {
              pending = new Map()
              pendingHits.set(agent, pending)
            }
            if (!pending.has(header.filename)) {
              pending.set(header.filename, { triggerPath: relPath, prefix: header.prefix! })
            }
          } else {
            ...
            andProgress.get(agent)!.set(header.filename, evaluation.remaining)
          }
        }
      }
    } catch (error) {
  ...
  // ── agent/pre-step：把 pending 命中的记忆全文注入下一次模型请求 ──
  ctx.on('agent/pre-step', async (
    ...
    try {
      ...
      const records = (await readAllMemories(options.memoryDirectory))
        .filter(record => fileNameSet.has(record.fileName))
      ...
      const composed = composeAssociateMessage(hits, trigger, Date.now(), options.maxTotalChars ?? MAX_ASSOCIATE_TOTAL_CHARS)
      if (composed.text === '') {
        pendingHits.delete(agent)
        return decision
      }
      ...
      // 只清掉已装入的记忆；因预算截断未装入的留待下次触发（预算外自然不再命中也接受）
      pendingHits.delete(agent)
...
/** 便捷工具：根据记忆目录推导项目根（<root>/.dsh/memory 形态 → <root>）。 */
export function deriveProjectRoot(memoryDirectory: string): string | undefined {
  const segments = splitPathSegments(memoryDirectory)
  // E:/DemoStudio/.dsh/memory → 去掉末尾 memory 与 .dsh 两段
  if (segments.length >= 2 && segments[segments.length - 1] === 'memory' && segments[segments.length - 2] === '.dsh') {
    return resolve(memoryDirectory, '..', '..')
  }
  return undefined
}
