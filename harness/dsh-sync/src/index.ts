/**
 * @demostudio/dsh-sync — DSH 启动同步插件。
 *
 ...
 * - ~/.dsh/memory           → <项目>/.dsh/memory            （dsh-memory 插件的记忆文件；若 memoryDir 钉在项目根则源为空，跳过）
 *
 * @module @demostudio/dsh-sync
 */
...
export const name = '@demostudio/dsh-sync'
...
export function apply(ctx: Context, config?: Config): void {
  ...
  if (!resolved.enabled) {
    ctx.logger.info('[dsh-sync] enabled=false，跳过同步')
    return
  }
  ...
  ctx.logger.info(
    `[dsh-sync] 完成: home=${resolved.homeDir} → ${targetRoot} | ` +
    `复制 ${total.copied} 个文件, 未变化 ${total.unchanged} 个, 删除 ${total.deleted} 个`,
  )
  if (total.copied > 0) {
    ctx.logger.info(`[dsh-sync] 变更文件: ${total.touched.join(', ')}`)
  }
}
