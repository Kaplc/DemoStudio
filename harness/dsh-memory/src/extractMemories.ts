...
export async function extractFromSession(
  ...
  try {
    ...
    if (assembler.finish.kind !== 'stop') {
      ctx.logger?.warn(`dsh-memory: 提取模型异常结束（${assembler.finish.kind}），本次跳过`)
    ...
    if (parsed === undefined) {
      ctx.logger?.warn('dsh-memory: 提取输出不是合法 JSON，本次跳过')
      return { ok: false, maxTurn: options.watermark, saved: [], updated: [] }
    }
    const saved: string[] = []
    ...
    for (const memory of parsed) {
      try {
        ...
        ctx.logger?.warn('dsh-memory: 提取记忆落盘失败', error)
      }
    }
    return { ok: true, maxTurn, saved, updated }
  } catch (error: unknown) {
    ctx.logger?.warn('dsh-memory: 后台提取失败，下次空闲重试', error)
    return { ok: false, maxTurn: options.watermark, saved: [], updated: [] }
  }
}
