...
> 项目：`@demostudio/dsh-memory`（`E:\DemoStudio\harness\dsh-memory\`）
> 日期：2026-08-30
...
- **要达成什么**：开发插件 `@demostudio/dsh-memory`，让 dsh agent 具备跨会话持久记忆能力——能显式保存/遗忘/检索/审查记忆，能在回合结束时自动提取值得记住的信息，能在每次请求时自动注入相关记忆。成功标准：会话 A 保存的记忆，在新会话 B 中能被检索注入并正确影响 agent 行为。
...
- 现有插件先例：`E:\DemoStudio\harness\dsh-engine-tools`（`@demostudio/dsh-engine-tools`，用 `ctx.tools.register` 注册工具）——了解项目插件的注册风格与包结构，但本插件采用正式 import + 类型（见下）。
...
**技术选型**：
- 语言 TypeScript/ESM，包名 `@demostudio/dsh-memory`，目录 `E:\DemoStudio\harness\dsh-memory\`（独立新包，与 `harness/dsh-engine-tools` 平级），`package.json` 中 `"dsh": {"bundle": {"patch": "./cordis.patch.yml"}}` 结构参考 `harness/dsh-engine-tools/package.json`。
...
- **兼容性**：不修改 dsh `agent-loop` 内核；所有行为走文档化扩展点；`cordis.yml` 留空数组占位即可，插件通过代码注册（参考 `harness/dsh-plugin/cordis.patch.yml` 注释说明）
...
- [ ] **lint/typecheck 通过**：`pnpm run typecheck`、`pnpm run lint`（在 harness/dsh-memory 包内）
...
1. **独立插件包** `@demostudio/dsh-memory`（`harness/dsh-memory/`），正式 import dsh 类型，不改 dsh 内核
