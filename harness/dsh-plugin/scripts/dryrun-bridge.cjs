...
server.listen(0, '127.0.0.1', async () => {
  ...
  // ─── 2) 准备 EngineContext（含 engineBridge + fileBridge），dsh-plugin 通过 ctx 注入 ───
  const engineBridge = {
  ...
  for (const tool of ALL_TOOLS) {
    try {
      ...
      // 真实调用 execute：dsh-plugin 工具签名 = (args, ctx)，schema 已 parse 过的 args
  ...
  // ─── 4) SSE 事件订阅验证（独立 connect，模拟 dsh-plugin 订阅 game lifecycle） ───
  console.log('\n[SSE] subscribing...')
