...
> 捕鱼达人（fish 项目）的关卡选择与进入流程：基地 HUD 双按钮 → 地图面板（关卡列表）→ 关卡空壳场景 → Esc 暂停菜单 → 返回基地。
...
> 相关文档：[`../engine/gameflow_system.md`](./engine/gameflow_system.md)、[`../engine/ui_system.md`](./engine/ui_system.md)、[`../engine/asset_tools_system.md`](./engine/asset_tools_system.md)。
