...
**已确认的调用方**：`setVelocity` 由 [TroopMoveComponent.ts:265](../../projects/fish/gameplay/battle/troops/TroopMoveComponent.ts) 每帧注入寻路速度；`onCollisionEnter` 由 [TroopMoveComponent.ts:58](../../projects/fish/gameplay/battle/troops/TroopMoveComponent.ts) 订阅（撞建筑）；`overlapTest` 由 [FishBaseGameMode.ts:355/742](../../projects/fish/gameplay/base/FishBaseGameMode.ts) 做建筑放置重叠判定（传 `{ group: CollisionLayer.BUILDING }`）；`queryAll` 由 [NavGrid.ts:70](../../src/engine/navigation/NavGrid.ts) 重建导航阻挡格。
...
| `setup(camera, uiEl)` / `setupUI(cam \| null)` | [PhySys.ts:82](../../src/engine/physics/PhySys.ts) / [:94](../../src/engine/physics/PhySys.ts) | 注入主相机 + 视口 DOM / 注入 UI 相机 | 前者阶段切换时调（[FishGameInstance.ts:622](../../projects/fish/gameplay/FishGameInstance.ts)）；UI 相机为 null 时 UI 层整体跳过 |
