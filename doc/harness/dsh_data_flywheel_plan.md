...
- **回合末纠正提示（2026-09-02 改，`preScreen.ts` 取代 side-query）**：独立 side-query 小模型判定被裁撤（对齐 ds-memory/ds-experience "删后台提取、主 agent 主动"方向，插件自身零 LLM 请求）。现机制：agent 空闲防抖 3s → 渲染增量转录 → 客户端纠正关键词预筛（`CORRECTION_HINT_PATTERN`，只测 `[用户] ` 行；未命中零成本推进水位）→ 命中则在**该 agent 的**规则段末尾挂"回合末纠正提示"（`SuspicionHint`：回合号 + 最近 2 条用户原话摘录），由主 agent 下一回合结合上下文自行判定双条件并走提案-确认制；未命中撤下提示。提示经 `AssembleContext.agent`（dsh-agent 扩展字段）按 agent 隔离，子 agent（delegationDepth>0）不预筛、无 agent 装配不渲染提示；`running` 撤销未触发定时器、水位留待下次空闲补检；`autoDetect: false` 关闭。配置 `extractModel`/`extractProvider` 删除，依赖 dsh-timeout 移除。
- 历史方案存档——**回合末自动检测（2026-08-31 补，`extractProposal.ts`，已于 2026-09-02 裁撤）**：依赖主模型自觉调 rule_propose 被证伪（落地首日经验库自动落 5 条 episode、规则库 0 提案），复刻 ds-experience 回合末骨架——agent 空闲防抖 3s → 客户端纠正关键词预筛（`CORRECTION_HINT_PATTERN`，只测 `[用户] ` 行；未命中零成本推进水位）→ 命中才发 side-query 小模型双条件判定（① 存在用户人工纠正；② 该纠正为此类任务正确完成的必要条件——不遵守就会做错/返工，一次性偏好不算）→ 双条件成立写 `pending/<name>.proposed.md` 并注入 notice（转述+等确认，绝不自动 apply）。提案-确认制不变，自动化只填 pending。判定模型 deepseek-chat/deepseek-official（config `extractModel`/`extractProvider` 可覆盖，`autoDetect: false` 关闭）；独立水位，子 agent 跳过，失败下次空闲重试。转录用本插件自持 `renderTurnTranscript` 副本（ds-memory/ds-experience/ds-feedback 三份同语义副本，插件间零依赖）。
- vitest：非法 name 拒绝 / propose→apply 全流程 / 同名无 mode 报错 / pending 不出现在规则段 / 预筛命中与不漏判 / 提示渲染与摘录截断 / 回合末接线（命中挂提示→下回合撤下、agent 隔离、子 agent 跳过、running 撤销补检、autoDetect 关闭）
