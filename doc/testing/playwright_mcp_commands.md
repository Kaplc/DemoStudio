...
> 相关文档：[内置浏览器工具速查](./playwright_commands.md)（另一条路径，能力等价） / [MCP 集成与调试桥](../editor/integration/mcp_integration.md) / [Playwright 测试流程](./playwright_testing.md) / [编辑器核心](../editor/core/core_system.md)
...
**边界划分**：页面内**调试桥的事件语义**归 [AI 事件系统](../engine/ai_system.md)；**MCP 工具转发**（`demostudio-editor` server）归 [MCP 集成与调试桥](../editor/integration/mcp_integration.md)——本文的 `playwright` server 与它是**两个不同的 MCP server**，不要混淆。
...
| `window.blueprintEditor` | 页面内蓝图编辑服务，见 [蓝图编辑](../editor/blueprint/blueprint_edit_system.md) |
...
另一条 AI 控制通道（不经浏览器，直接控制 Electron）见 [MCP 集成与调试桥](../editor/integration/mcp_integration.md)：
